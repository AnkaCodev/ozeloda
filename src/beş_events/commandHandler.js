...
const beş_embed = new EmbedBuilder().setColor(`#2f3136`).setAuthor({name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true, size: 2048 })}).setFooter({text: beş_config.clientPresence ? beş_config.clientPresence : `Anka Code` , iconURL: message.author.avatarURL({ dynamic: true, size: 2048 })})
if (cmd) {
